...
int rscuad::rscuad_manager::manager_init()
{
    ...
    ROS_INFO("Manager initial loaded ...");
...
int rscuad::rscuad_manager::dxl_process(){
    ...
    while(1) {
        ...
        dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, DXL_ID, ADDR_GOAL_POSITION, dxl_goal_position[indexs], &dxl_error);
        #else
        dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, DXL_ID, ADDR_GOAL_POSITION, dxl_goal_position[indexs], &dxl_error);
        #endif
...
void rscuad::rscuad_manager::move_robot(char *str){
    ...
    ROS_WARN("0 : %f",atof(joint_0));
    ROS_WARN("1 : %f",atof(joint_1));
    ROS_WARN("2 : %f",atof(joint_2));
    ROS_WARN("3 : %f",atof(joint_3));
    ROS_WARN("4 : %f",atof(joint_4));
    ROS_WARN("5 : %f",atof(joint_5));
    ROS_WARN("6 : %f",atof(joint_6));
    ROS_WARN("7 : %f",atof(joint_7));
    ROS_WARN("8 : %f",atof(joint_8));
    ROS_WARN("9 : %f",atof(joint_9));
    ROS_WARN("10 : %f",atof(joint_10));
    ROS_WARN("11 : %f",atof(joint_11));
    ROS_WARN("12 : %f",atof(joint_12));
    ROS_WARN("13 : %f",atof(joint_13));
